...
class RepositoryCodeGraph:
    ...
    def __init__(self, repo: Path, cache_path: Path):
        self.repo = repo.resolve(); self.cache_path = cache_path
    ...
    def build(self) -> dict[str, Any]:
        ...
        for path in self.repo.rglob("*"):
            if not path.is_file() or any(part in ignored for part in path.relative_to(self.repo).parts): continue
            relative = path.relative_to(self.repo).as_posix()
            if path.suffix not in {".py", ".js", ".ts", ".tsx", ".jsx", ".go", ".rs", ".java", ".cs", ".toml", ".yaml", ".yml", ".json"}: continue
            try: file_hashes[relative] = sha256_file(path)
            ...
            if path.suffix == ".py":
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.Import): imports.extend(alias.name for alias in node.names)
                        ...
                        nodes.append(CodeNode(id=f"symbol:{relative}:{node.name}", kind="class" if isinstance(node, ast.ClassDef) else "function",
                                              path=relative, name=node.name, line=getattr(node, "lineno", 0)))
                if imports:
                    nodes[-(1 if nodes else 0):]
                    # Store imports on the file node.
                    for item in reversed(nodes):
                        if item.id == f"file:{relative}": item.imports = sorted(set(imports)); break
        data = {"files": file_hashes, "nodes": [node.to_dict() for node in nodes]}
        self.cache_path.parent.mkdir(parents=True, exist_ok=True); self.cache_path.write_text(json.dumps(data, separators=(",", ":")), encoding="utf-8")
        return data

    def load(self) -> dict[str, Any]:
        if not self.cache_path.exists(): return self.build()
        try: return json.loads(self.cache_path.read_text(encoding="utf-8"))
        except json.JSONDecodeError: return self.build()

    def symbols_for_paths(self, paths: list[str]) -> list[str]:
        wanted = {Path(path).as_posix() for path in paths}
        data = self.load()
        return sorted({str(node["name"]) for node in data.get("nodes", []) if node.get("kind") != "file" and node.get("path") in wanted})

    def search_symbols(self, query: str, limit: int = 20) -> list[dict[str, Any]]:
        lower = query.lower(); matches = []
        for node in self.load().get("nodes", []):
            if node.get("kind") == "file": continue
            name = str(node.get("name", ""))
            if name.lower() in lower or lower in name.lower(): matches.append(node)
