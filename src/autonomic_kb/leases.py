...
from .util import atomic_write, parse_time, sha256_text, utc_now
...
class LeaseStore:
    ...
    def acquire(self, task: str, agent: str, scope: str = "repository", ttl_minutes: int = 30) -> TaskLease:
        ...
        lease = TaskLease(
            lease_id=f"lease:{uuid.uuid4().hex}", task_signature=signature, agent=agent, scope=scope,
            started=now.replace(microsecond=0).isoformat().replace("+00:00", "Z"),
            expires=(now + timedelta(minutes=max(1, ttl_minutes))).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
            artifacts=[],
        )
    ...
    def find(self, task_or_signature: str) -> TaskLease | None:
        signature = task_or_signature if len(task_or_signature) == 20 and " " not in task_or_signature else sha256_text(task_or_signature.strip().lower())[:20]
