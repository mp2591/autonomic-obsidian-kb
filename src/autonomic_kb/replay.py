...
class MatchedReplayHarness:
    ...
    def _run_one(self, task: str, mode: str, argv: list[str], cwd: Path, timeout: float) -> TaskOutcome:
        ...
        env.update(
            {
                "KB_MODE": mode,
                "KB_TASK": task,
                "KB_VAULT": str(self.config.vault),
        ...
        outcome = TaskOutcome(
            task_id=f"replay:{task_hash}:{mode}:{utc_now()}",
            task_hash=task_hash,
            kb_mode=mode,
            success=success,
            ...
            searches=int(metrics.get("searches", 0)),
            file_reads=int(metrics.get("file_reads", 0)),
            commands=int(metrics.get("commands", 0)),
            retries=int(metrics.get("retries", 0)),
            corrections=int(metrics.get("corrections", 0)),
            latency_ms=round(elapsed, 3),
            maintenance_tokens=int(metrics.get("maintenance_tokens", 0)),
    ...
    def run(self, spec_path: str | Path) -> dict[str, Any]:
        ...
        for case in spec.get("tasks", spec if isinstance(spec, list) else []):
            task = str(case["task"])
            cwd = Path(case.get("cwd") or self.config.repo or Path.cwd()).resolve()
            timeout = float(case.get("timeout", 900))
            baseline = [str(x) for x in case["baseline_command"]]
            assisted = [str(x) for x in case["kb_command"]]
            a = self._run_one(task, "no-kb", baseline, cwd, timeout)
            b = self._run_one(task, "kb", assisted, cwd, timeout)
