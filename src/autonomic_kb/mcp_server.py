...
class MCPServer:
    ...
    def tools(self) -> list[dict[str, Any]]:
        return [
            ...
            {
                "name": "kb_validate",
                "description": "Run schema, evidence, provenance, temporal, safety, and executable validators.",
    ...
    def call_tool(self, name: str, arguments: dict[str, Any]) -> Any:
        with KnowledgeIndex(self.config) as index:
            ...
            if name == "kb_evidence_get":
                record = EvidenceStore(self.config).get(str(arguments["id"]))
                return record.to_dict() if record else {"error": "not-found"}
            if name == "kb_validate":
                return Validator(self.config, index).validate().to_dict()
            if name == "kb_why":
                index.index_vault()
            ...
            if name == "kb_status":
                return dashboard_data(self.config)
    ...
    def read_resource(self, uri: str) -> Any:
        if uri == "kb://dashboard":
            return dashboard_data(self.config)
        if uri == "kb://evidence":
            return {"ids": EvidenceStore(self.config).list_ids()}
        if uri == "kb://memories":
            with KnowledgeIndex(self.config) as index:
                index.index_vault()
                return {
        ...
        if uri.startswith("kb://memory/"):
            ...
            with KnowledgeIndex(self.config) as index:
                index.index_vault()
                return index.get(memory_id) or {"error": "not-found"}
        if uri.startswith("kb://evidence/"):
            evidence_id = uri[len("kb://evidence/") :]
            record = EvidenceStore(self.config).get(evidence_id)
            return record.to_dict() if record else {"error": "not-found"}
