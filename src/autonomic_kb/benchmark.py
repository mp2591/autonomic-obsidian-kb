    ...
    def run(self, tasks_path: str | Path) -> dict[str, Any]:
        ...
        with KnowledgeIndex(self.config) as index:
            ...
            for case in tasks:
                ...
                results.append(
                    BenchmarkCaseResult(
                        name=str(case.get("name", case["task"][:40])),
                        task=str(case["task"]),
                        budget=budget,
        ...
        summary = {
            ...
            "mean_expected_coverage": round(
                sum(result.expected_coverage for result in results) / max(1, len(results)), 4
            ),
...
class TraceBenchmarkRunner:
    ...
    def run(self) -> dict[str, Any]:
        ...
        return {
            ...
            "mean_read_delta": round(sum(item["read_delta"] for item in items) / max(1, len(items)), 2),
            "items": items,
        }
