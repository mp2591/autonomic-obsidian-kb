...
class ValidationReport:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"created_at": self.created_at, "scanned": self.scanned, "errors": self.errors, "warnings": self.warnings,
...
class ValidatorRegistry:
    ...
    def run(self, metadata: dict[str, Any], relative: str) -> list[ValidationIssue]:
        ...
        for spec in validators:
            ...
            if handler:
                ...
                result.append(ValidationIssue("warning", "unknown-validator", relative, f"Unknown validator kind {kind!r}",
                                              str(metadata.get("id", "")), False, {"validator": spec}))
        return result
    ...
    def _file_exists(self, metadata: dict[str, Any], spec: dict[str, Any], relative: str) -> list[ValidationIssue]:
        ...
        if not path:
            return [ValidationIssue("error", "validator-path-escape", relative, "file-exists validator escaped repository root",
                                    str(metadata.get("id", "")), False, spec)]
        if not path.exists():
            return [ValidationIssue("warning", "validator-file-missing", relative, f"Validator path does not exist: {spec.get('path')}",
                                    str(metadata.get("id", "")), True, spec)]
        return []
    ...
    def _source_hash(self, metadata: dict[str, Any], spec: dict[str, Any], relative: str) -> list[ValidationIssue]:
        ...
        if not path:
            return [ValidationIssue("error", "validator-path-escape", relative, "source-hash validator escaped repository root",
                                    str(metadata.get("id", "")), False, spec)]
        if not path.exists():
            return [ValidationIssue("warning", "validator-file-missing", relative, f"Validator source is missing: {spec.get('path')}",
                                    str(metadata.get("id", "")), True, spec)]
        if expected and sha256_file(path) != expected:
            return [ValidationIssue("warning", "validator-hash-mismatch", relative, f"Validator source hash changed: {spec.get('path')}",
                                    str(metadata.get("id", "")), True, spec)]
        return []
    ...
    def _command(self, metadata: dict[str, Any], spec: dict[str, Any], relative: str) -> list[ValidationIssue]:
        ...
        if not self.config.repo:
            return [ValidationIssue("info", "validator-skipped", relative, "Command validator requires --repo", memory_id, False, spec)]
        ...
        if isinstance(command, str):
            ...
            return [ValidationIssue("error", "invalid-validator", relative, "command validator requires argv", memory_id, False, spec)]
        if not argv:
            return []
        ...
        if Path(argv[0]).name not in {Path(str(value)).name for value in allowed}:
            return [ValidationIssue("error", "validator-not-allowlisted", relative, f"Executable {argv[0]!r} is not allowlisted",
                                    memory_id, False, spec)]
        forbidden = {";", "&&", "||", "|", ">", "<", "`", "$()"}
        if any(any(token in arg for token in forbidden) for arg in argv):
            return [ValidationIssue("error", "validator-shell-syntax", relative, "Shell syntax is not allowed in executable validators",
                                    memory_id, False, spec)]
        timeout = min(30.0, max(0.5, float(spec.get("timeout", 10.0))))
        try:
            result = subprocess.run(argv, cwd=self.config.repo, text=True, capture_output=True, timeout=timeout, check=False)
        except (OSError, subprocess.TimeoutExpired) as error:
            return [ValidationIssue("warning", "validator-exec-failed", relative, f"Validator execution failed: {error}", memory_id, True, spec)]
        expected = int(spec.get("expected_exit", 0))
        if result.returncode != expected:
            detail = (result.stderr or result.stdout or "")[-300:]
            return [ValidationIssue("warning", "validator-command-failed", relative,
                                    f"Validator exited {result.returncode}, expected {expected}: {detail}", memory_id, True, spec)]
        contains = str(spec.get("stdout_contains", ""))
        if contains and contains not in result.stdout:
            return [ValidationIssue("warning", "validator-output-mismatch", relative,
                                    f"Validator output did not contain {contains!r}", memory_id, True, spec)]
        return []
...
class Validator:
    ...
    def _load_schema(self) -> dict[str, Any]:
        candidates = [Path(__file__).resolve().parents[2] / "schema" / "memory.schema.json", self.config.vault / "schema" / "memory.schema.json"]
    ...
    def validate(self) -> ValidationReport:
        ...
        for memory_id, paths in id_paths.items():
            if len(paths) > 1:
                for relative in paths:
                    issues.append(ValidationIssue("error", "duplicate-id", relative,
        ...
        for path, metadata, text in notes:
            ...
            for target in parsed.links:
                if not self._resolve_link(target, path_set, basename_map):
                    issues.append(ValidationIssue("warning", "broken-link", relative, f"Unresolved wikilink [[{target}]]",
        ...
        self.index.events.emit("validate.completed", scanned=report.scanned, errors=report.errors,
    ...
    def validation_priorities(self) -> list[dict[str, Any]]:
        ...
        risk_weight = {"agent-instruction": 1.0, "command": 0.8, "workflow": 0.75, "invariant": 0.9,
                       "dependency": 0.7, "fact": 0.45}
        for note in self.index.all_notes({"active", "stale"}):
            ...
            result.append({"id": note["declared_id"] or note["id"], "path": note["path"], "priority": round(priority, 4),
                           "days": round(days, 1), "uses": uses})
        return sorted(result, key=lambda item: (-item["priority"], item["id"]))

    def _paths(self):
        ignored = {".git", ".obsidian", ".kb", ".kb-evidence", ".kb-memory-events", ".kb-episodes", "__pycache__", ".venv"}
    ...
    def _schema_issues(self, path: str, metadata: dict[str, Any]) -> list[ValidationIssue]:
        ...
        for key in required:
            if key not in metadata or metadata.get(key) in {None, ""}:
                issues.append(ValidationIssue("error" if key in {"id", "type", "scope", "summary"} else "warning",
        ...
        if metadata.get("type") and metadata["type"] not in VALID_TYPES:
            issues.append(ValidationIssue("error", "invalid-type", path, f"Unknown memory type {metadata['type']!r}", memory_id))
        if metadata.get("kind") and metadata["kind"] not in VALID_KINDS:
            issues.append(ValidationIssue("error", "invalid-kind", path, f"Unknown memory kind {metadata['kind']!r}", memory_id))
        if metadata.get("scope") and metadata["scope"] not in VALID_SCOPES:
            issues.append(ValidationIssue("error", "invalid-scope", path, f"Unknown scope {metadata['scope']!r}", memory_id))
        if metadata.get("status") and metadata["status"] not in VALID_STATUSES:
            issues.append(ValidationIssue("error", "invalid-status", path, f"Unknown status {metadata['status']!r}", memory_id))
        try:
            confidence = float(metadata.get("confidence", 0.5))
            if not 0 <= confidence <= 1:
                raise ValueError
        except (TypeError, ValueError):
            issues.append(ValidationIssue("error", "invalid-confidence", path, "confidence must be between 0 and 1", memory_id, True))
        schema_version = int(metadata.get("schema_version", 1) or 1)
        if schema_version < 2:
            issues.append(ValidationIssue("info", "legacy-schema", path, "Memory uses schema v1; migrate when semantically edited", memory_id, True))
        elif self.schema and jsonschema is not None:
            try:
                jsonschema.Draft202012Validator(self.schema).validate(metadata)
            except jsonschema.ValidationError as error:
                issues.append(ValidationIssue("error", "json-schema", path, error.message, memory_id, True,
                                              {"json_path": list(error.absolute_path)}))
        if int(metadata.get("token_cost", 0) or 0) > 1600:
            issues.append(ValidationIssue("warning", "oversized-memory", path,
                                          "Memory exceeds 1,600 estimated tokens; split or compile a smaller task view", memory_id))
        return issues

    @staticmethod
    def _security_issues(path: str, memory_id: str, text: str) -> list[ValidationIssue]:
        return [ValidationIssue(finding.severity, finding.category, path, f"{finding.pattern}: {finding.excerpt}",
                                memory_id, True, finding.to_dict()) for finding in scan_content(text)]
    ...
    def _evidence_issues(self, path: str, metadata: dict[str, Any]) -> list[ValidationIssue]:
        ...
        if not isinstance(raw, list):
            return [ValidationIssue("error", "invalid-evidence", path, "evidence must be a list", str(metadata.get("id", "")))]
        issues = []
        for evidence_id in raw:
            if not self.evidence.verify(str(evidence_id)):
                issues.append(ValidationIssue("warning", "evidence-missing-or-tampered", path,
                                              f"Evidence {evidence_id!r} is missing or failed digest verification",
                                              str(metadata.get("id", "")), True, {"evidence_id": evidence_id}))
        return issues
    ...
    def _invalidation_issues(self, path: Path, metadata: dict[str, Any]) -> list[ValidationIssue]:
        ...
        if isinstance(source_hashes, dict) and self.config.repo:
            ...
            for source, expected in source_hashes.items():
                ...
                try:
                    ...
                    issues.append(ValidationIssue("error", "source-path-escape", relative,
                                                  f"Validation source {source!r} escapes repository root", memory_id, False))
                    continue
                if not absolute.exists():
                    issues.append(ValidationIssue("warning", "source-missing", relative,
                                                  f"Validation source {source!r} no longer exists", memory_id, True, {"source": source}))
                elif expected and sha256_file(absolute) != str(expected):
                    issues.append(ValidationIssue("warning", "source-changed", relative,
                                                  f"Validation source {source!r} changed after validation", memory_id, True, {"source": source}))
        expires = str(invalidation.get("expires", "") or "")
        parsed = parse_time(expires)
        if parsed and parsed < datetime.now(timezone.utc):
            issues.append(ValidationIssue("warning", "expired", relative, f"Memory expired at {expires}", memory_id, True))
        patterns = invalidation.get("paths", [])
        if isinstance(patterns, str):
            patterns = [patterns]
        if patterns and self.config.repo:
            repo_files = [item.relative_to(self.config.repo).as_posix() for item in self.config.repo.rglob("*")
                          if item.is_file() and ".git" not in item.parts]
            for pattern in patterns:
                if not any(fnmatch.fnmatch(item, str(pattern)) for item in repo_files):
                    issues.append(ValidationIssue("info", "obsolete-path", relative,
        ...
        if metadata.get("freshness") == "verified" and not source_hashes and age_days(str(metadata.get("validated", ""))) > self.config.stale_after_days:
            issues.append(ValidationIssue("warning", "validation-aged", relative,
                                          f"Unbound validation is older than {self.config.stale_after_days} days", memory_id, True))
        return issues
