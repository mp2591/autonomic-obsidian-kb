...
def compile_task_view(note: dict[str, Any], task: str, preferred_layer: int, max_tokens: int, profile: str = "generic") -> tuple[int, str]:
    ...
    for layer in range(preferred_layer, -1, -1):
        ...
        if estimate_tokens(text, profile) <= max_tokens:
            return layer, text
        task_terms = set(terms(task)); chunks = [chunk.strip() for chunk in _SENTENCE.split(text) if chunk.strip()]
        scored = []
        for position, chunk in enumerate(chunks):
            chunk_terms = set(terms(chunk)); overlap = len(task_terms & chunk_terms) / max(1, len(task_terms))
            exact_bonus = 0.35 if _CODE.search(chunk) else 0.0
            identifier_bonus = 0.15 if any(token in chunk for token in task_terms if any(c in token for c in "_./:")) else 0.0
        ...
        for _, position, chunk in sorted(scored, key=lambda item: (-item[0], item[1])):
            ...
            if cost + used > max_tokens: continue
            selected.append((position, chunk)); used += cost
            if used >= max_tokens * 0.8: break
        if selected:
            selected.sort(); compiled = "\n\n".join(chunk for _, chunk in selected)
            if compiled.strip(): return layer, compiled.strip()
