...
class RetrievalTests(unittest.TestCase):
    ...
    def test_budget_progressive_disclosure_and_why(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            config = make_vault(Path(temporary))
            write_memory(
                config, "commands/test.md", "kb:repository:command:test", "Test command",
                "Run python -m unittest discover to execute the complete test suite", memory_type="command",
    ...
    def test_module_and_branch_scope_are_hard_gates(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            config = make_vault(Path(temporary))
            write_memory(
                config, "module.md", "kb:module:fact:api", "API module", "special API behavior",
                scope="module", module="api", applies_to=["api/**"],
            )
            write_memory(
                config, "branch.md", "kb:branch:fact:experiment", "Branch behavior", "special API behavior",
                scope="branch", branch="experiment",
            )
            write_memory(
                config, "repo.md", "kb:repository:fact:general", "General API", "special API behavior",
            )
            with KnowledgeIndex(config) as index, patch("autonomic_kb.retrieval.inspect_git", return_value=self._git()):
                manifest = Retriever(config, index).retrieve(
                    "special API behavior", budget=300, paths=["src/index.py"]
                )
    ...
    def test_graph_expansion_can_add_the_specific_solution(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            config = make_vault(Path(temporary))
            write_memory(
                config, "failure.md", "kb:repository:known-failure:sqlite", "SQLite lock failure",
                "sqlite database locked during indexing", memory_type="known-failure",
                relations={"fixed-by": ["kb:repository:solution:connection-ownership"]},
                applies_to=["src/**"],
            )
            write_memory(
                config, "solution.md", "kb:repository:solution:connection-ownership", "Connection ownership",
                "Use deterministic connection ownership and context managers", memory_type="solution",
                applies_to=["src/**"],
            )
            with KnowledgeIndex(config) as index, patch("autonomic_kb.retrieval.inspect_git", return_value=self._git()):
                manifest = Retriever(config, index).retrieve(
